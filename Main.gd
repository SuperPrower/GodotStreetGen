...
func run_net():
	...
	evaluate(placedObjects)

func evaluate(objects: Dictionary):
	score = 0
	...
	for key in objects.keys():
		if objects[key] == "LAMP":
			lamps.append(key)
		elif objects[key] == "VENDING_MACHINE":
			vmachines.append(key)

	if vmachines[0].distance_to(vmachines[1]) < 3:
		score += 25

	var lamp_distances = [
		lamps[0].distance_to(lamps[1]),
		lamps[0].distance_to(lamps[2]),
		lamps[1].distance_to(lamps[2]),
	]

	lamp_distances.sort()

	if abs(lamp_distances[0] - lamp_distances[1]) < 0.01:
		score += 25
